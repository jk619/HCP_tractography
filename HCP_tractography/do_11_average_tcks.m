...
trans = 0
dors = load_nifti('/usr/local/fsl/data/standard/MNI152_T1_1mm.nii.gz')
vent = load_nifti('/usr/local/fsl/data/standard/MNI152_T1_1mm.nii.gz')
...
%             system(sprintf('applywarp -v -i %s -r %s -w %s  -o %s%s',[newname(1:end-4) '.nii.gz'], standard_MNI,anat2standard,[newname(1:end-4) '_MNI.nii.gz']));
            ...
            if p == 1
            dors.vol(:,:,:,ct) = tmp_nat.vol >0
            else
                vent.vol(:,:,:,ct) = tmp_nat.vol>0
            end
...
%%
vent.vol(vent.vol == 0) = NaN;
vent.vol = nansum(vent.vol,4);
save_nifti(vent,'./average_maps/ventral.nii.gz')
dors.vol(dors.vol == 0) = NaN;
dors.vol = nansum(dors.vol,4);
save_nifti(dors,'./average_maps/dorsal.nii.gz')